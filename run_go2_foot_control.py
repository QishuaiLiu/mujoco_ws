...
class Go2FootPositionController(Go2StandController):
    ...
    def _update_scripted_foot_targets(self) -> None:
        ...
        if (
            self.swing_leg_name is None
            and self.swing_diagonal_name is None
            and not self.alternate_diagonal_swing
        ):
            return
...
def main() -> None:
    ...
    parser.add_argument(
        "--alternate-diagonal",
        action="store_true",
        help="Alternate the swing phase between FL+RR and FR+RL.",
    )
    parser.add_argument(
        "--trot-forward",
        action="store_true",
        help="Run an open-loop forward trot with swing and stance foot motion.",
    )
    parser.add_argument(
        "--step-length",
        type=float,
        default=0.06,
        help="Peak-to-peak x motion in meters for --trot-forward.",
    )
    parser.add_argument(
        "--step-height",
        type=float,
        default=0.025,
        help="Foot lift height in meters for --trot-forward.",
    )
    parser.add_argument(
        "--gait-frequency",
        type=float,
        default=0.5,
        help="Gait cycle frequency in Hz for --trot-forward.",
    )
    parser.add_argument(
        "--swing-fraction",
        type=float,
        default=0.5,
        help="Fraction of each gait cycle spent in swing for --trot-forward.",
    )
    parser.add_argument(
        "--print-tau-every",
        type=int,
    ...
    if args.trot_forward:
        controller.configure_forward_trot(
            args.step_length,
            args.step_height,
            args.gait_frequency,
            args.swing_fraction,
